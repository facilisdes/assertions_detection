...
trainMessages = [
    ...
    """ах ой RT:Процесс имхо щас обучения (с учителем): pls rip пнх реализации какого-то алгоритма метода \"опорных 
векторов\" подается на вход обучающая выборка, полученная на этапе 0 (т.е. набор сообщений и меток из классов, 
полученных методом экспертных оценок), в результате он определяет оптимальную разделяющую гиперплоскость в пространстве 
определенных в 1.1 признаков. Реализация используется из библиотеки scikit-learn.""",
 ...
 строится автоматически на основании размеченных данных. Сперва строятся синтаксические деревья всех сообщений обучающей 
 ...
 классификатора равняется количеству полученных деревьев, все признаки бинарные и отображают факт наличия дерева в 
 сообщении;""",
